...
JOIN 
    popular p ON vl.video_id = p.video_id`;
...
FROM
    users u
JOIN
    channel_subscribe cs ON u.user_id = cs.user_id
JOIN
    ...
    const query = `
       SELECT
        ...
        JOIN
            popular p ON vl.video_id = p.video_id
        WHERE
            vl.video_title LIKE ? OR c.channel_name LIKE ?;
    `;
...
app.get('/watch', (req, res) => {
    console.log(req.query); // Logs query parameters (e.g., video ID)
    const { v, u } = req.query; // 'v' likely for video_id, 'u' for user_id (for subscription check)

    if (!v) { // Checks if video ID is provided
    ...
    // The first '?' is for user_id in the subquery, the second '?' is for video_id in the main WHERE clause
    db.query(query, [u, v], (err, result) => { // Assuming 'u' is user_id from query params
        if (err) {
            console.error('เกิดข้อผิดพลาดในการค้นหา:', err); // Original error message
            res.status(500).send('เกิดข้อผิดพลาดในการค้นหา'); // Original error message
        } else {
            // Processing results to split comments
            const video = result[0]; // Assuming only one video result for a given video_id
            if (video && video.comments) {
                video.comments = video.comments.split('\n').map(comment => {
                    const [username, content] = comment.split(':');
                    return { username, content };
                });
            } else {
                video.comments = []; // No comments or video not found
            }
            res.json(video);
        }
    });
